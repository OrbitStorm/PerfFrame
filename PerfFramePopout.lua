-- PerfFrame: Settings Dropdown
-- Provides: PerfFrameTemplates.CreatePFPopout(parent, entries, callback)

PerfFrameTemplates = PerfFrameTemplates or {}

local function SetDropdownHover(frame)
    if frame.Button:IsMouseOver() and not frame.Popout:IsShown() then
        ...
        local hl = frame.Button:GetHighlightTexture()
...
local function AddPopoutEntry(parent, label, value)
...
local function CreateNextBackButton(parent, kind, anchor)
    local button = CreateFrame("Button", nil, parent)
    button:SetSize(32, 32)
    button:SetNormalAtlas("charactercreate-customize-" .. kind .. "button")
    button:SetPushedAtlas("charactercreate-customize-" .. kind .. "button-down")
    button:SetHighlightAtlas("charactercreate-customize-" .. kind .. "button")
    button:SetDisabledAtlas("charactercreate-customize-" .. kind .. "button-disabled")
    button:SetScript("OnMouseDown", function()
        button:SetHighlightAtlas("charactercreate-customize-" .. kind .. "button-down")
    end)
    button:SetScript("OnClick", function()
        parent:Select(kind == "back" and parent.selectedIndex - 1 or parent.selectedIndex + 1)
    end)
...
local function SetSelectedEntry(frame, entry)
...
function PerfFrameTemplates.CreatePFPopout(parent, entries, callback)
    ...
    frame.Button:SetScript("OnEnter", function() SetDropdownHover(frame) end)
    frame.Button:SetScript("OnLeave", function() SetDropdownHover(frame) end)
    ...
    frame.Button:SetScript("OnEvent", function(_, _, buttonID)
        if frame.Popout:IsShown() then
            local hide = true
            if frame.Button:IsMouseOver() then
                hide = false
            else
    ...
    if NineSlicePanelMixin and NineSlicePanelMixin.OnLoad then
    ...
    if entries then
        ...
        for idx, entry in ipairs(entries) do
            ...
            if entry.selected then selectedIndex = idx end
        end
